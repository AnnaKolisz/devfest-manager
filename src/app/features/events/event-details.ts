...
export class EventDetails {
  // Open src/app/app.config.ts and confirm that withComponentInputBinding() 
  // is included in the router provider. 
  // This is the mechanism that turns route parameters into Component Inputs.
  // musze to sprawdzić zanim mogę użć tego rozwiązania
  readonly id = input.required<string>();
  readonly eventService = inject(EventsService);
  //WHen passing a Signal to HttpResources, you should padd entire signal references
  // without invoking it. This allows the resources to reactively respong to changes
  // in  the signal value. Invoking the signal would only pass the current value
  // at that moment losing the reactive behavior.
